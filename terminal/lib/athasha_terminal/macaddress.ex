defmodule MACAddress do
  ...
  defp extract_mac_addresses(if_addresses) do
    Enum.reduce(if_addresses, [], fn {if_id, if_data}, acc ->
  ...
  defp extract_address_from(if_addresses, interface_name) do
    ...
    Enum.reduce_while(if_addresses, nil, fn {if_id, if_data}, acc ->
  ...
  def to_hex(mac_address, opts \\ []) when is_mac_address(mac_address) do
    ...
    do_to_hex(
      mac_address |> Base.encode16(case: base_case) |> to_charlist(),
      [],
      separator |> to_charlist()
    )
  end
