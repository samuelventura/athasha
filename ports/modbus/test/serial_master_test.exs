defmodule Modbus.SerialMasterTest do
  ...
  test "serial master test" do
    ...
    res = <<0x50, 1, 1, 0x00>>
    val = [0]
    pp1(master, serial, cmd, req, res, val, state)
    ...
    pp2(master, serial, cmd, req, res, state, state2)
    ...
    :ok = Serial.stop(serial)
  end

  defp pp1(master, serial, cmd, req, res, val, state) do
    spawn(fn ->
      length = Request.length(cmd) + 2
      {:ok, rtu_req} = Serial.readn(serial, length)
  ...
  defp pp2(master, serial, cmd, req, res, state, state2) do
    spawn(fn ->
      length = Request.length(cmd) + 2
      result = Serial.readn(serial, length, 800)
      {:ok, rtu_req} = result
      ^req = Wrapper.unwrap(rtu_req)
      ^rtu_req = Wrapper.wrap(req)
      {^cmd, nil} = Protocol.parse_req(rtu_req)
